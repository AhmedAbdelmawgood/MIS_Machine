...
class Thread    // Thread Class
{
	...
	public:
        ...
        bool setToCore(int core_id); // Clear affinity and set it to the cpu with core_id
        bool addCore(int core_id); // Add the cpu with core_id to the group of cpus the thread should be scheduler on.
        int getCoreCount(); // Return the number of cores available in the system
