...
// Check to see if the provided core_id is less the the maximum number of available cpus/cores
    if ( core_id < cpu_count ){
    CPU_ZERO(&cpuset); // Clear the cpuset mask
    return addCore(core_id); // invoke addCore to add the core with index core_if to the set
    } // if not return false
    else return false;
}
...
bool Thread::addCore(int core_id)
    ...
    if (pthread_setaffinity_np(pthread, sizeof(cpu_set_t), &cpuset) == 0 ) return true;
    else return false; // We return false if pthread_setaffinity_np failed
    } // if not return false
    else return false;
}

int Thread::getCoreCount() { return cpu_count; }
